...
import static com.sparta.givemetuna.domain.user.entity.Role.TEAM_MANAGER;
...
public class BoardService {
	...
	public List<BoardListResponseDto> checkAvailableBoard(User user) {
		List<Board> boardList = boardRepository.findAll();
		List<Board> availableBoards = boardList.stream()
			.filter(board -> board.getInvitedUserRole().stream()
				.anyMatch(boardUserRole -> boardUserRole.getUser().equals(user))
			)
			.toList();
//        List<Board> availableBoards = boardList.stream()
//			.filter(board -> board.getInvitedUserRole().stream()
//				.filter(
//					boardUserRole -> boardUserRole.getUser().equals(user))
//				.isParallel()
//			).toList();
